defmodule ExploringMars.MissionRunner do
  ...
  def get_bounds_and_run(input, output) do
    case read_bounds(input) do
      ...
      {_, msg} ->
        IO.write(output, "Invalid bounds: " <> msg <> "\n")
    end
  end
  ...
  @typep position_error ::
           {:invalid_position, list(String.t())}
           | {:no_parse, String.t()}
           | IO.nodata()
  ...
  defp read_position(device) do
    with line when line != :eof <- IO.read(device, :line),
         [sx, sy, dir] <- String.split(line, " ") do
  ...
  @spec read_instructions(File.io_device()) ::
          {:ok, list(Instruction.t())}
          | IO.nodata()
